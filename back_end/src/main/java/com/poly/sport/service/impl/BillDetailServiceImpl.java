...
//        }
//
//        // Check inventory for CHO_XAC_NHAN status
//        if (bill.getStatus() == BillStatusConstant.CHO_XAC_NHAN) {
//            int availableStock = shoeDetail.getQuantity();
//            // Calculate total quantity of this product in the bill
...
//        }
//
//        // Save BillDetail (no inventory deduction)
...
//
//        // Delete BillDetail (no inventory addition)
//        billDetailRepository.delete(billDetail);
//
//        // Update bill totals and log history if necessary
//        if (bill.getStatus() != BillStatusConstant.TAO_DON_HANG) {
//            Double calculateTotalMoney = 0.0;
//            for (BillDetail x : billDetailRepository.findByBillId(bill.getId())) {
//                calculateTotalMoney += x.getQuantity() * x.getPrice().doubleValue();
//            }
//            bill.setTotalMoney(BigDecimal.valueOf(calculateTotalMoney).subtract(bill.getMoneyReduce() != null ? bill.getMoneyReduce() : BigDecimal.ZERO));
//
...
//                bill.getStatus() == BillStatusConstant.CHO_XAC_NHAN ||
//                bill.getStatus() == BillStatusConstant.CHO_THANH_TOAN ||
//                bill.getStatus() == BillStatusConstant.XAC_NHAN_THONG_TIN_THANH_TOAN) {
//            BillHistory billHistory = new BillHistory();
//            billHistory.setBill(bill);

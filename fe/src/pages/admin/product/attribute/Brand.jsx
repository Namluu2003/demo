...
function Brand() {
  ...
  const [pageSize, setPageSize] = useState(5); // Sửa từ `pageBrand` thành `pageSize`
  ...
  useEffect(() => {
    loadData(currentPage, pageSize, searchValue, statusBrand); // Sửa từ `pageBrand` thành `pageSize`
  }, [currentPage, pageSize, searchValue, statusBrand]); // Sửa từ `pageBrand` thành `pageSize`

  const loadData = (currentPage, pageSize, searchValue, statusBrand) => {
    // Sửa từ `pageBrand` thành `pageSize`
    request
      .get("/brand", {
        params: {
          name: searchValue,
          page: currentPage,
          sizePage: pageSize, // Sửa từ `pageBrand` thành `pageSize`
          status: statusBrand,
        },
      })
      .then((response) => {
    ...
    <div className="brand-container">
        ...
        <Row gutter={10}>
          <Col span={16}>
            ...
            <Select
              defaultValue={null}
              style={{ width: "100%" }}
              onChange={(value) => setStatusBrand(value)}
            >
      ...
      <div className="card p-3">
        <div className="d-flex justify-content-between align-items-center mb-2">
          <h6 className="fw-semibold">Bảng thương hiệu</h6>
          <Link to={"/admin/brand"}>
            <Button
              type="primary"
        ...
        <Table
          ...
          pagination={{
            ...
            current: currentPage,
            pageSize: pageSize, // Sửa từ `pageBrand` thành `pageSize`
            // pageSizeOptions: [5, 10, 20, 50, 100], // Sửa từ `pageBrandOptions` thành `pageSizeOptions`
            // showQuickJumper: true,
            total: totalPages * pageSize, // Sửa từ `pageBrand` thành `pageSize`
            onChange: (page, pageSize) => {
              // Sửa từ `pageBrand` thành `pageSize`
              setCurrentPage(page);
              setPageSize(pageSize); // Sửa từ `setPageBrand` thành `setPageSize`
            },
          }}
        />

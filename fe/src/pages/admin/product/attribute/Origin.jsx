...
function Origin() {
  ...
  const [statusOrigin, setStatusOrigin] = useState(null);
  ...
  useEffect(() => {
    loadData(currentPage, pageSize, searchValue, statusOrigin);
  }, [currentPage, pageSize, searchValue, statusOrigin]);

  const loadData = (currentPage, pageSize, searchValue, statusOrigin) => {
    request
      .get("/xuatxu", {
        params: {
          name: searchValue,
  ...
  const handleAdd = (data) => {
    confirm({
      title: "Xác nhận",
      icon: <ExclamationCircleFilled />,
      content: "Bạn có chắc muốn thêm xuất xứ mới? ",
      okText: "Xác nhận",
      okType: "danger",
      cancelText: "Hủy",
      onOk() {
  ...
  const handleUpdate = (data) => {
    confirm({
      title: "Xác nhận",
      icon: <ExclamationCircleFilled />,
      content: "Bạn có chắc muốn cập nhật tên xuất xứ? ",
      okText: "Xác nhận",
      okType: "danger",
      cancelText: "Hủy",
      onOk() {
        request
          .put(`/xuatxu/${item.id}`, data)
          .then((response) => {
            if (response.status === 200) {
              console.log(response);
      ...
      <div className="origin-container">
          ...
          <Row gutter={10}>
            ...
            <Col span={8}>
              ...
              <Select
                defaultValue={null}
                style={{ width: "100%" }}
                onChange={(value) => setStatusOrigin(value)}
              >
        ...
        <div className="card p-3">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <h6 className="fw-semibold">Bảng xuất xứ</h6>
            <Link to={"/admin/origin"}>
              <Button
                type="primary"
          ...
          >
            <Form onFinish={handleAdd} layout="vertical" form={formAdd}>
              ...
              >
                <Input placeholder="Nhập tên xuất xứ..." />
              </Form.Item>
